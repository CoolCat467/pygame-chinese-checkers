...
def checkJump(moves: list, board: dict, destination: tuple, direction: tuple, playerNum: int):
    ...
    for dir_ in DIRECTIONS:
        jumpDir = mult(dir_, 2)
        if dir_ == mult(direction, -1) or add(destination, dir_) not in board or add(destination, jumpDir) not in board or add(destination, jumpDir) in moves:
            continue
        elif board[add(destination, dir_)] is None:
            continue
        else:
        ...
        if dest in moves:
            ...
            #out of bounds or two pieces in a line
            continue
        moves.append(dest)
        try:
            #recursively checks available squares
            checkJump(moves, board, dest, dir_, playerNum)
